...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_gtt', help='training zhongjian image name', required=True)
    ...
    parser.add_argument('--noise_amp',type=float,help='addative noise cont weight',default=0)
    parser.add_argument('--batchSize', type=float, default=4)
    opt = parser.parse_args()

    netG = model.senet(opt).to(opt.device)
    ...
    gtt_data=sio.loadmat('%s/%s' % (opt.input_dir, opt.input_gtt))['gtt']
    gtt_data=functions.matRead(gtt_data,opt)
    ...
    for i in range(opt.epoch):
        ...
        for j in range(1000):
            gtt_image, gt_image = functions.getBatch2se(gtt_data, gt_data, opt.batchSize)
            netG.zero_grad()
            fake_image = netG(gtt_image)
        ...
        schedulerG.step()
        torch.save(netG.state_dict(), os.path.join('SEOutput/G_epoch_{}.pth'.format(i)))
    end_time = time()
